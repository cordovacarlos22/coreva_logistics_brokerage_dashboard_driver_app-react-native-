...
export async function createDraftLoad(
  supabase,
  { driverId, loadNumber, originAddress, destinationAddress, consigneeId = null }
) {
  const { data, error } = await supabase
    .from('loads')
    .insert({
...
// Extra stops beyond the primary origin/destination (CLAUDE.md: "a single
// load can have multiple deliveries/appointments"). load_stops_write RLS
// already permits this via the parent load's driver_id, no schema change
// needed. `sequence` starts at 2 -- 1 is implicitly the primary
// destination, which lives on loads.destination_address, not a stops row.
export async function addLoadStops(supabase, loadId, stops) {
  ...
  if (error) throw new Error(`load_stops insert: ${error.message}`);
  return data;
}

...
//   photo of strapped load -> checklist_photos (type: load_secured), AI
//                             compliance check via /api/vision/load-secured
...
// under Hermes.
//
// Mirrors uploadBolPhoto: uploads to Storage, then calls the backend's
// /api/vision/load-secured route, which downloads it, runs the AI
// compliance check (single criterion: straps/wrap visibly crossing the
// load), and inserts the checklist_photos row itself with the verdict
// already attached -- checklist_photos has no UPDATE policy, so the
// verdict has to land at INSERT time, same reason the BOL route inserts
// its own row rather than leaving it to the client. A "fail" verdict is
// a normal response here, not a thrown error -- see lib/uploadQueue.js's
// looksLikeNetworkFailure, which this deliberately doesn't trip.
export async function uploadLoadSecuredPhoto(supabase, { loadId, checklistId, uri, mimeType }) {
  ...
  return apiPost('/api/vision/load-secured', { loadId, checklistId, storagePath });
}

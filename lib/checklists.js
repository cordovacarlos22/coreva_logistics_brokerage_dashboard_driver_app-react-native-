...
// Scan New Shipment's "photo first" flow: runs Textract on the BOL photo
// before any load/checklist exists yet, so it can't use the checklistId-
// scoped path convention above. Uploaded under `staging/` instead -- the
// `bol-photos` insert policy only checks is_driver(), no path restriction,
// so this is allowed even with nothing else created yet. See
// finalizeBolPhoto below for what happens to this path once the load does
// exist.
export async function previewBolPhoto(supabase, { driverId, uri, mimeType }) {
  const arraybuffer = await fetch(uri).then((res) => res.arrayBuffer());
  const contentType = mimeType || 'image/jpeg';
  const extension = contentType.split('/')[1] || 'jpg';
  const storagePath = `staging/${driverId}/bol-${Date.now()}.${extension}`;
  ...
  const result = await apiPost('/api/ocr/bol/preview', { storagePath });
  return { storagePath, ...result };
}

// Applies fields already extracted by an earlier previewBolPhoto call to a
// load that's now been created -- deliberately doesn't re-upload the photo
// or re-run Textract, both already happened at preview time.
export async function finalizeBolPhoto(supabase, { loadId, checklistId, storagePath, fields, raw }) {
  return apiPost('/api/ocr/bol/attach', { loadId, checklistId, storagePath, fields, raw });
}

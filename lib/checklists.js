...
// supabase/README.md for the `load-photos`/`bol-photos` storage buckets.
// This is the real Hub Group departure-checklist workflow (see the Pickup
// flow screen), digitized onto the one-row-per-driver-per-load
// `checklists` table:
//   step 0 "confirm arrival"          -> arrived_at
...
//   step 6 "upload BOL"               -> checklist_photos (type: bol), plus
//                                        loads.bol_* via the /api/ocr/bol
//                                        backend route (see lib/api.js)
//   step 7 "confirm departure"        -> departed_at, plus loads.status
...
// /api/ocr/bol route, which downloads it from Storage, runs Google Cloud
// Vision, writes the extracted fields onto the load, and inserts the

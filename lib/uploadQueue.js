...
import { uploadBolPhoto, uploadLoadSecuredPhoto, finalizeBolPhoto } from './checklists.js';
...
const UPLOAD_FNS = {
  bol: uploadBolPhoto,
  bolAttach: finalizeBolPhoto,
  load_secured: uploadLoadSecuredPhoto,
  pod: uploadPodPhoto,
};
...
// kind: 'bol' | 'bolAttach' | 'load_secured' | 'pod'. args match the
// corresponding upload function's shape (uploadBolPhoto / finalizeBolPhoto /
// uploadLoadSecuredPhoto / uploadPodPhoto). 'bolAttach' args are plain JSON
// (no local file uri), so unlike the others it isn't subject to the cache-
// reclaim limitation described above.
